...
const MySolution = ({ index, uid, doc, id, testResults, failedTest, userInfo, verifiedTests, config, isAdmin, variables, defaultResult, code, dispatch }) => {
...
function mapStateToProps(state, ownProps) {
    ...
    const { code, defaultResult, activeFailedTestID, testResults } = user.solutions[id];
    let failedT = null;
    ...
    return update(ownProps, { index: { $set: index }, defaultResult: { $set: defaultResult }, testResults: { $set: testResults }, failedTest: { $set: failedTest }, userInfo: { $set: userInfo }, verifiedTests: { $set: verifiedTests }, config: { $set: config }, uid: { $set: uid }, id: { $set: id }, isAdmin: { $set: isAdmin }, variables: { $set: variables }, code: { $set: code }, doc: { $set: doc } });
}
